...
def _team_table_html(team: dict) -> str:
    games = _sort_games((team.get("pbp_entry") or {}).get("games") or [])
    if not games:
    ...
    for game in games:
        ...
        opponent = game.get("opponent_abbr") or game.get("opponent") or "OPP"
        wk = game.get("week") or game.get("game_number") or "?"
        result = "W" if margin is not None and margin > 0 else "L"
        score_txt = f"{int(pf)}-{int(pa)}" if pf is not None and pa is not None else "N/A"
        yds = game.get("total_yards", "N/A")
...
def _team_md(team: dict) -> str:
    ...
    for game in games:
        ...
        yds = game.get("total_yards", "N/A")
        lines.append(
            f"- Wk {wk} vs {opponent}: {result} {score_txt} | Yds {yds} | PFΔ {_fmt_delta(pf_delta)} | PAΔ {_fmt_delta(pa_delta, inverse=True)} | {badge_text}"
        )
